...
It's pretty much a virtual DOM, in that the [YngwieElement](#yngwieElement) class can represent DOM [elements](https://developer.mozilla.org/en-US/docs/Web/API/Element), can create and manipulate those elements without them being part of the DOM, and can render those elements into actual DOM nodes. It can also do the reverse though, using [YngwieTransform](#yngwieTransform) to generate YngwieElements from existing DOM nodes, or generate YngwieElements from a STRING of HTML markup, e.g. `<div>HTML Content</div>`
 ...
 Yngwie node relationships are implemented as a linked list, which is why there is no "children" property. Instead, every [YngwieNode](#yngwieNode) instance, and it's subclasses, have references to the first child, last child, next sibling, and previous sibling. There is a "children" method though that returns an array of nodes by iterating the first child's siblings until there aren't any siblings left to iterate.
 ...
 The YngwieController class handles binding event listeners to elements at time of render. Event listeners bound this way also have access to the yngwieElement that the element that's listening for events was generated from - this opens up the possibility of re-rendering parts of the UI , without re-rendering the entire UI, when certain events are dispatched by the user or another controller.
...
### CONSTRUCTOR :: STRING, OBJECT, STRING, [yngwieController] -> yngwieNode
...
### yngwieElement.on :: STRING, [(EVENT, this, NODE -> VOID)]|EVENT, this, NODE -> VOID -> this
Chainable method that binds event listeners by event name to element at render.
...
// Bind click events:
elem.on("click", [
  (evt, elem, node) => alert(elem.text()),
  (evt, elem, node) => console.log(elem)
])

// Bind mouseover event:
elem.on("mouseover", (evt, elem, node) => console.log(node))
```
...
### yngwieElement.render :: STRING|NODE|VOID -> NODE
Transforms this element and it's descendants into a browser DOMElement NODE.
Optional argument determines where to append render to, otherwise DOMElement NODE is returned. Optional argument can either be a NODE or STRING to query NODE for, whereas that query is applied to entire DOM.
...
### YngwieElement.renderTo :: STRING|NODE, [yngwieElement] -> NODE
Renders an array of yngwieElements and appends result to given target, returning the node those elements were appended to. If any element of the given array is not an instance of YngwieElement, a YngwieError exception is thrown. If given argument is not an array, a YngwieError exception is also thrown.
...
### YngwieElement.inject :: STRING|NODE, yngwieElement -> NODE
Replaces the given target with the render of the given YngwieElement. If given argument is
not an instance of YngwieElement, then a YngwieError exception is thrown.
